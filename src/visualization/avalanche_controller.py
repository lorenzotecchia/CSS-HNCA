...
class AvalancheController:
    ...
    n_neurons: int
    quiet_threshold: float = 0.0
    stimulus_fraction: float = 1.00
    stimulus_min: int = 1
    ...
    _pause_remaining: float = field(default=0.0, init=False, repr=False)
    ...
    def rebind(self, simulation: Simulation, n_neurons: int, reset_seen: bool = False) -> None:
    ...
    def apply_stimulus(self) -> None:
        if self.n_neurons <= 0:
            return
        count = max(self.stimulus_min, int(self.n_neurons * self.stimulus_fraction))
        count = min(count, self.n_neurons)
    ...
    def record_step(self, time_step: int, firing_count: int) -> bool:
        ...
        if self.seen >= self.target:
            return False
        self._pause_remaining = max(0.0, self.pause_seconds)
